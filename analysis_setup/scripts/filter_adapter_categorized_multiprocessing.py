...
import sys, os
import pysam
import cy_stringscan, cy_bamindexer
import re
...
def find_adapter_anywhere(seq, pat, mm_max=1, allow_del=True):
    ...
    n = len(seq)
    if n >= L and mm_max >= 0:
        new_hits = cy_stringscan.hamming_scan(seq, pat, mm_max)
        hits.extend(new_hits)

    # exactly 1 deletion in adapter
    if allow_del and n >= L - 1:
        new_hits = cy_stringscan.deletion_scan_vector(seq, pat)
        hits.extend(new_hits)
...
# ---------------- multiprocessing queue thread -------
    
def bam_indexer(queue_in_bam, path_in_bam):
    with pysam.AlignmentFile(path_in_bam, "rb") as inf:
        offset_start = inf.tell()
        read_idx = 0

        for _read in inf: ###read in from input BAM file
            if read_idx % CHUNK_SIZE == 0:
                offset_end = inf.tell()
                queue_in_bam.put((offset_start, offset_end))
                offset_start = offset_end
            read_idx += 1
        
        queue_in_bam.put((offset_end, inf.tell()))


def file_writer(path_extract_out, queue_extractions, n_workers):
    with open(path_extract_out, "w") as tsv_out:
        write_extract_header(tsv_out)
        sentinel_counter = 0
        while sentinel_counter < n_workers: 
            entry = queue_extractions.get()
            if entry is not None:
                tsv_out.writelines(entry)
            else:
                sentinel_counter += 1

        tsv_out.close()

def process_alignments(inf, outf, process_queues, analysis_parameters):
    queue_in_bam = process_queues['queue_in_bam']
    queue_extractions = process_queues['queue_extractions']
    queue_stats = process_queues['queue_stats']

    adapter_fwd = analysis_parameters['adapter_fwd']
    adapter_rc = revcomp(adapter_fwd)
    min_mapq = analysis_parameters['min_mapq']
    mm_max = analysis_parameters['mm_max']
    allow_del = analysis_parameters['allow_del']

    extract_n = analysis_parameters['extract_n']
    fullread_classify = analysis_parameters['fullread_classify']
    ...
    while True:
        if inf.tell() >= offset_end: #get new chunk if end reached. use also to send tsv
            offset_start, offset_end = queue_in_bam.get()
            if offset_start is not None and offset_end is not None:
                if offset_end == 0:
                    raise Exception
            queue_extractions.put(emit_list)
            emit_list = [] 

            if offset_start is None: ###break if sentinel
                queue_extractions.put(emit_list)
                queue_extractions.put(None) #sentinel for writer process
                break

            inf.seek(offset_start)
        ...
        if softclip_total == 1:
            if len(hits5) == 1:
                ...
                if extract_n > 0 and sc5:
                    ...
                    emit_list.append(f"{r.query_name}\t{"5p"}\t{mode.decode("utf-8")}\t{s}\t{e}\t{extracted}\n")
                ...
                if extract_n > 0 and sc3:
                    ...
                    emit_list.append(f"{r.query_name}\t{"3p"}\t{mode.decode("utf-8")}\t{s}\t{e}\t{extracted}\n")
    ...
    queue_stats.put(stats)
    outf.close()

def worker_process(path_in_bam, path_out_bam, ID, process_queues, analysis_parameters):
    out_path = re.sub(r'\.bam$', "temp" + str(ID) + ".bam", path_out_bam)
    with pysam.AlignmentFile(path_in_bam, "rb") as inf, \
        pysam.AlignmentFile(out_path, "wb", header=inf.header) as outf:
        process_alignments(inf, outf, process_queues, analysis_parameters)
...
def process_bam(path_in_bam, path_out_bam, n_threads, analysis_parameters):
    queue_in_bam = Queue(maxsize=100)
    queue_extractions = Queue(maxsize=100)
    queue_stats = Queue()

    worker_process_queues = {
        "queue_in_bam" : queue_in_bam,
        "queue_extractions" : queue_extractions,
        "queue_stats" : queue_stats
    }

    n_threads = max(n_threads, 3)
    n_workers = n_threads - 2

    reader_process = Process(
        target=cy_bamindexer.get_virtual_offsets, 
        args = (path_in_bam.encode("utf-8"), CHUNK_SIZE, queue_in_bam))
    ...
    for i in range(n_workers):
        worker_processes.append(
            ...
            args=(path_in_bam, path_out_bam, i, 
                    worker_process_queues, analysis_parameters),
            ))
    ...
    extract_n = analysis_parameters['extract_n']
    extract_out = analysis_parameters['extract_out']
    if extract_n > 0:
        writer_process = Process(
                target=file_writer, 
                args=(extract_out, queue_extractions, n_workers)
            )       
        
        writer_process.start()  

    reader_process.join() #wait here until file is read

    if reader_process.exitcode != 0:
        terminate_all(reader_process, *worker_processes, writer_process)
        raise RuntimeError(f"Reader Process exited with code {reader_process.exitcode}\nTerminating all processes.")
    else:
        print("Reader process exited successfully.")

    for _ in range(n_workers):
        queue_in_bam.put((None, None)) ##sentinel

    for index, process in enumerate(worker_processes):
        process.join() # wait for all still running processes
        if process.exitcode != 0:
            terminate_all(reader_process, *worker_processes, writer_process)
            raise RuntimeError(f"Reader process {index} exited with code {process.exitcode}\nTerminating all processes.")
        else:
            print(f"Worker Process {index} exited successfully.")

    if extract_n > 0:
        writer_process.join()
        if writer_process.exitcode != 0:
            terminate_all(reader_process, *worker_processes, writer_process)
            raise RuntimeError(f"Writer process exited with code {writer_process.exitcode}\nTerminating all processes.")
        else:
            print("Writer process exited successfully.")

    stats = queue_stats.get() # get first stat counter
    stats_received = 1
    while stats_received < n_workers:
        stats += queue_stats.get()
        stats_received += 1

    print("Reads processed:", stats['total'])
...
def main():
    ...
    ap.add_argument("--n-threads", type=int, default=1, help="Number of threads for parallel processing (> 4!!)")
    ...
    analysis_parameters = {
        "adapter_fwd" : args.adapter,
        "min_mapq" : args.min_mapq,
        "mm_max" : args.mm_max,
        "allow_del" : (not args.no_del),
        "fullread_classify" : args.fullread_classify,
        "extract_n" : args.extract_n,
        "extract_out" : args.extract_out
    }

    n_threads = max(args.n_threads, 3)
    n_workers = n_threads - 2

    stats = process_bam(
        args.input_bam, args.output_bam, n_threads,
        analysis_parameters = analysis_parameters
    )

    path_temp_bams = [re.sub(r'\.bam$', "temp" + str(i) + ".bam", args.output_bam) for i in range(n_workers)]
    pysam.cat("-o", args.output_bam,*path_temp_bams)
    for i in range(n_threads-2):
        os.remove(re.sub(r'\.bam$', "temp" + str(i) + ".bam", args.output_bam))
